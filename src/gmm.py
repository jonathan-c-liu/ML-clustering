...
class GMM():
    ...
    def _e_step(self, features):
        ...
        """
        p = np.zeros([features.shape[0], self.n_clusters])
        for i in range(self.n_clusters):
            p[:, i] = self._posterior(features, i)
        return p
    ...
    def _m_step(self, features, assignments):
        ...
        for j in range(self.n_clusters):
            gamma[j] = np.sum(assignments[:, j])
            for i in range(features.shape[0]):
                means[j, :] += assignments[i, j] * features[i, :]
                covariances[j, :] += assignments[i, j] * np.square(features[i, :] - self.means[j, :])
            means[j, :] = means[j, :] / gamma[j]
            covariances[j, :] = covariances[j, :] / gamma[j]

        mixing_weights = gamma / features.shape[0]
    ...
    def _log_likelihood(self, features, k_idx):
        ...
        return np.log(self.mixing_weights[k_idx]) + logpdf
